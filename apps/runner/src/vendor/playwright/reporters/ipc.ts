...
import { displayObj } from '../../../utils';

export type JSONIPCReport = {
  messageType: 'report';
  report: JSONReport;
};

/**
 * Variant of the JSONReporter that sends the JSONReport object back to the parent process.
 */
class JSONIPCReporter implements Reporter {
  config!: FullConfig;
  suite!: Suite;
  tests!: TestCase[];
  ...
  constructor() {
    if (process.channel) {
      console.log(
        'Process has an IPC channel established. Report will be sent on test completion.'
      );
    }
  }
  ...
  async onEnd(result: FullResult) {
    await outputReport(this._serializeReport(result));
  }

  /** TODO
   *
   * From https://playwright.dev/docs/api/class-reporter#reporter-on-exit:
   *
   * Called immediately before test runner exists.
   * At this point all the reporters have received the reporter.onEnd() signal, so all the reports should be build.
   * You can run the code that uploads the reports in this hook.
   */
  ...
  private _locationMatches(
    s1: JSONReportSuite | JSONReportSpec,
    s2: JSONReportSuite | JSONReportSpec
  ) {
    return (
    ...
    for (const spec of from.specs || []) {
      const toSpec = to.specs.find(
        (s) =>
          s.title === spec.title &&
          s.file ===
          toPosixPath(path.relative(this.config.rootDir, spec.file)) &&
          s.line === spec.line &&
...
async function outputReport(report: JSONReport) {
  const ipcReport: JSONIPCReport = { messageType: 'report', report };

  // Send the JSON report back to the parent process
  if (process.channel) {
    const sendReport = new Promise<void>((res, rej) => {
      process.send!(ipcReport, undefined, undefined, (err) => {
        if (err) {
          rej(err);
        } else {
          res();
    ...
    console.log(
      '\nNo IPC channel detected. Check JSON report for test results.'
    );
  }

  const output = process.env.APOLLO_RUNNER_JSON_OUTPUT_PATH;
  if (output) {
    await fs.promises.mkdir(path.dirname(output), { recursive: true });
    await fs.promises.writeFile(output, displayObj(report, 2));
  }
}
...
export default JSONIPCReporter;